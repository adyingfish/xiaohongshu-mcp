...
通过 MCP 自己管理的浏览器和现有 Cookie 登录态操作小红书网页消息页面。支持会话查询、收件人与正文预览，以及向已有会话或没有聊天记录的陌生人发送一条文字私信；无需安装 XHS Bridge。
...
接口继承现有 `AUTH_TOKEN` / `-token` 鉴权配置。`user_id` 是主页或会话中的 24 位用户 ID，不是昵称或小红书号。查询仅覆盖网页当前加载的单人会话，`complete: false` 不代表没有更多会话。向陌生人发起单人文字私信已覆盖；批量读取陌生人收件箱、群聊、图片、视频及文件消息不在本功能范围。

已有会话可先查询收件人；陌生人可直接使用已核对的用户 ID 和完整昵称预览，无须对方先出现在会话列表中。随后取得用户对收件人与正文的发送授权后发送。预览不会填写或发送，也不会保存跨请求草稿；发送会重新核对用户 ID、会话类型、完整昵称、正文和输入框状态。打开消息页或会话可能被网站自动标记已读，返回 `read_may_mark_seen: true`。
...
预览和发送统一使用官网“发消息”按钮对应的 `/chat?openUid=<用户ID>` 入口。网站由此初始化没有聊天记录的新会话；仅访问 `/chat/<用户ID>` 不会完成该步骤。两种 URL 都必须与网页选中的单人会话 ID、列表昵称和会话标题同时匹配，重复、非法或冲突的 `openUid` 参数会被拒绝。

打开会话最多等待 30 秒，等待发送确认最多 20 秒。列表和预览的服务入口预算为 50 秒；发送在此基础上增加逐字输入和四次停顿的预算，服务入口和浏览器动作使用同一规则：63 秒 + 正文 Unicode 字符数 × 400 毫秒，最长正文最多 463 秒。需要发送长消息时，客户端调用超时应设为至少 480 秒（例如 mcporter 的 `--timeout 480000`）；原有 60 秒客户端超时不会自动延长。调用方更早取消时沿用更短期限。会话控件就绪后保留页面加载屏障，再重新核对收件人、输入框和最近消息。

会话未就绪时明确返回“尚未填写或发送”；已经尝试发送但未确认的结果仍为 `unknown`，不能自动重试。输入框被网站禁用时停止，不绕过平台或收件人的私信限制。
...
发送使用现有 `humanize` 节奏：会话核验后短暂停留，正文逐字插入并加入随机间隔，输入后与提交前停顿，服务端确认成功后再短暂停留。相同草稿不重复输入，预览和列表读取不增加这些发送动作；状态轮询仍使用固定间隔。

阅读停顿后会重新核对收件人、草稿和最近消息。提交前的停顿发生在原子核验之外，核验与 Enter 触发仍在同一次脚本执行中。发送前取消会停止后续动作，逐字输入中断可能留下部分草稿，需核对后再操作；发送成功后的停顿被取消不会覆盖已确认结果。发送总时限按正文字符数增加输入预算，仍服从调用方更短的超时。
...
DM_TEST_BROWSER=/absolute/path/to/chrome \
  go test -tags integration ./xiaohongshu -run '^TestDirectMessage(Stranger)?BrowserFixture$' -v
# 全部集成测试只编译，不执行账号操作。
go test -tags integration -run '^$' ./...
```

离线浏览器测试分别覆盖已有会话与通过 `openUid` 延迟建立的陌生人会话，包括只读预览、单次发送、服务端确认、失败、结果未知、昵称不符、禁用输入框和已有草稿。
