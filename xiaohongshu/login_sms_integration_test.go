...
func TestLoginSMSBrowserFixture(t *testing.T) {
 ...
 <section id="sms"><h3>短信验证码验证</h3><p>验证码已发送至 +86 151******15</p><input id="sms-code" placeholder="请输入验证码"><span>没有收到验证码？</span><a onclick="window.resends++">获取验证码</a><div class="submit"><span>验证</span></div><span class="error"></span></section>
 <script>window.submits=0;window.wrongClicks=0;window.resends=0;window.inputEvents=0;document.querySelector('#sms-code').addEventListener('input',()=>window.inputEvents++);document.querySelector('.submit').onclick=()=>{window.submits++;window.received=document.querySelector('#sms-code').value;document.querySelector('.error').textContent='验证码错误';};</script>
 </body>`)
	})
	go router.Run()
	...
	require.Error(t, a.SubmitSMSCode(ctx, "invalid"))
	...
	require.Error(t, a.SubmitSMSCode(ctx, "654321"))
	require.Equal(t, 1, p.MustEval(`() => window.submits`).Int())
